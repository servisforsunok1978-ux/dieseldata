#!/usr/bin/env python3
"""Одноразовий bootstrap: дамп public.continental_injector -> ІСНУЮЧА Google-таблиця.

Аналогічно bootstrap_solenoid.py / bootstrap_vehicles.py. Сервісний акаунт без
Workspace не може створити Drive-файл (403), тож таблицю створює людина/конектор
під власним акаунтом і ділиться з СА (Редактор), а цей скрипт наповнює вкладку
`continental_injector`.
...
Env:
  GOOGLE_SERVICE_ACCOUNT_JSON, SHEET_ID_CONTINENTAL (або --sheet-id),
  SUPABASE_DB_PASSWORD (+HOST/USER/PORT/NAME).

Вигружаються всі 5 колонок (generated-колонок у таблиці немає), у порядку
ordinal_position:
  oe_number, oem_continental, nozzle, washer, nut
"""
import argparse
import json
import os
...
SHEET_TAB = 'continental_injector'
EXPORT_COLS = ['oe_number', 'oem_continental', 'nozzle', 'washer', 'nut']
...
def fetch_rows(conn_params):
    ...
    with psycopg2.connect(**conn_params) as conn:
        with conn.cursor() as cur:
            cur.execute(f'SELECT {cols_sql} FROM public.continental_injector '
                        f'ORDER BY oe_number;')
            return cur.fetchall()
...
def main(argv=None):
    ...
    p = argparse.ArgumentParser(description='Bootstrap continental_injector -> existing Google Sheet')
    p.add_argument('--create', action='store_true', help='підтвердити реальний запис')
    p.add_argument('--sheet-id', default=os.environ.get('SHEET_ID_CONTINENTAL'),
    ...
    print(f'Прочитано з continental_injector: {len(data)} рядків.')
    ...
    print('Далі: постав цей id у GitHub variable SHEET_ID_CONTINENTAL і прожени '
          'sync-continental з dry_run=true для перевірки round-trip.')
